...
class DataBase:
    ...
    def connect(self, pathDataBase:str):
        ...
        try:
            ...
            con = sqlite3.connect(pathDataBase, check_same_thread=False)
            return con
        except Error:
            print(Error)
    ...
    def get_users(self, count=20, remote_users=False):
        ...
        :return: ('038dfb1f-fc43-4ac7-bf4d-55f9d732b59c', 'Шумелев', 'Дмитрий', 'Игореви', 1, None, 0)
        '''
        try:
            cursor = self.con.cursor()
    ...
    def add_user(self, last_name, first_name, middle_name, mode_skip=1, status_photo=0, personId=None):
        '''
    ...
    def get_descriptors(self):
        '''
        Получить все дискрипторы пользователй
        :return:
        '''
        try:
            cursor = self.con.cursor()

            sql = "SELECT personId, descriptors FROM users where not descriptors is NULL AND del is NULL"
            cursor.execute(sql)
            rows = cursor.fetchall()

            #формируем данные для обучения
            encodings, person_id = [], []
            for personId, descriptor in rows:
                descriptors = json.loads(descriptor)
        ...
        return person_id, encodings

    def pull_descriptors(self, personId,  descriptors):
        '''
        ...
        cursor = self.con.cursor()
        cursor.execute(
            "UPDATE users SET descriptors='{}' where personId = '{}' ".format(descriptors, personId))
        self.con.commit()

        return 0

    def getInformation_by_personID(self, personId):
        '''
        ...
        try:
            ...
            return {'personId': personId, 'last_name': last_name, 'first_name': first_name, 'middle_name': middle_name, 'mode_skip': mode_skip}
...
if __name__ == '__main__':
    pathDataBase = '/home/dima/PycharmProjects/recognition_door_web/rc/database'
    ...
    # from app_web.moduls.trening_models_cvm_knn import get_encodings_and_personId_by_image
    # descriptor_1 = get_encodings_and_personId_by_image('/home/dima/Изображения/Веб-камера/2020-09-18-133602.jpg')
    # descriptor_2 = get_encodings_and_personId_by_image('/home/dima/Изображения/Веб-камера/2020-09-18-133611.jpg')
    import json
    #
    # descriptor = [list(descriptor_1), list(descriptor_2)]
    # print("{}".format(json.dumps(descriptor)))


    # database.pull_descriptors('56b790bc-bbaa-4ca5-90d8-7b49a4161c43', str(json.dumps(descriptor)))
    # data = database.get_descriptors()
    person_id, encodings = database.get_descriptors()
    # encodings, person_id = [], []
    # for personId, descriptor in data:
    #     descriptors = json.loads(descriptor)
    #     print("de", personId, len(descriptors))
    #     for descriptor in descriptors:
    #         encodings.append(descriptor)
    #         person_id.append(personId)
    # print(person_id)
    from app_web.moduls.trening_models_cvm_knn import train_cvm, train_knn, save_model
